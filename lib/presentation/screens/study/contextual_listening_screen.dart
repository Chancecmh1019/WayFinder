  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('情境聽力', style: TextStyle(color: fg, fontSize: 18)),
        centerTitle: true,
        actions: [
          Padding(
      ...
      body: learnedWords.when(
        data: (words) {
          if (_words.isEmpty && words.isNotEmpty) {
            _words = words.take(10).toList()..shuffle();
            _generateOptions();
          }
          ...
          if (_words.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.headphones_outlined, size: 64, color: AppTheme.gray400),
        ...
        children: [
          ...
          Center(
            child: GestureDetector(
              onTap: () => _playAudio(currentWord.lemma),
              child: Container(
                width: 140,
                height: 140,
          ...
          Text(
            _hasPlayedOnce ? '選擇你聽到的單字' : '點擊播放發音',
          ...
          // 提示：定義（只在顯示結果後才顯示）
          if (_showResult && currentWord.senses.isNotEmpty)
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    currentWord.senses.first.zhDef,
          ...
          // 確認按鈕
          ElevatedButton(
            onPressed: (_selectedAnswer == null || !_hasPlayedOnce) && !_showResult ? null : () {
              if (_showResult) {
                _nextQuestion();
              } else {
                _checkAnswer();
  ...
  List<Widget> _buildOptions(VocabularyEntity word, bool isDark, Color card, Color fg) {
    return _currentOptions.map((option) {
      ...
      return Padding(
        ...
        child: GestureDetector(
          onTap: _showResult || !_hasPlayedOnce ? null : () {
